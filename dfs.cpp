...
using namespace std;
    ...
    //Searches using a stack like fringe
    //Works exatcly like BFS but implements a stack
list<int*> DepthFirstSearch::search(int* currentState){
    ...
    while(!isGoalState(currentState)){  // The main loop for running the search
        if(fringe.empty()){ // Either makes the first state, path pair, or gets one from the fringe
            ...
            currentElement = fringe.front();    //Stack functionality
            fringe.pop_front();
